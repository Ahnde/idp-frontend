...
/***/ function(module, exports) {
	...
	module.exports = "<div><label for=\"{{id}}\" class=\"control-label form-label {{to.labelSrOnly ? 'sr-only' : ''}}\" ng-if=\"to.label\">{{to.label}}\n{{to.required ? '*' : ''}}</label><formly-transclude></formly-transclude><label for=\"{{id}}\" class=\"control-label form-postlabel {{to.labelSrOnly ? 'sr-only' : ''}}\" ng-if=\"to.postLabel\">{{to.postLabel}}\n{{to.required ? '*' : ''}}</label> <div class='validation validation-block' ng-if='options.validation.errorExistsAndShouldBeVisible' ng-messages='options.formControl.$error'><div ng-messages-include='validation.html'></div><div class=\"validation-message\" ng-message='{{::name}}' ng-repeat='(name, message) in ::options.validation.messages'>{{message(options.formControl.$viewValue, options.formControl.$modelValue, this)}}</div></div>"
/***/ },
/* 8 */
/***/ function(module, exports) {
